...
FROM athletes
WHERE id in (SELECT DISTINCT athlete_id, athlete_ FROM event_results WHERE team_id in (SELECT id FROM teams WHERE team='Jamaica'))
...
SELECT medals.medal
FROM medals
WHERE medals.medal LIKE 'gold%' and medals.owner LIKE 'Greg Louganis'
ORDER BY medals.game ASC;
